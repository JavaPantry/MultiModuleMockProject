 ...
 */
Ext.define('Fast.store.sales.SalesMonthFctEditExt', {
	extend: 'Ext.data.Store'
	,grid: null // set by MonthEditGridExt.init()
    ,requires: ['Ext.data.reader.Json']
    ,model: 'Fast.model.sales.SalesMonthFctModelExt'
    ,storeId:'Fast.store.sales.SalesMonthFctEditExt'
    ,pageSize: 35
    ,autoLoad: false
    ,proxy: {
        type: 'ajax'
	...
	,updateHeader:function( records ){
    	//console.log ('SalesMonthFctEditExt:updateHeader');
	...
	,listeners: {//see: src\data\AbstractStore
	    load: function( store, records, successful, eOpts ){
	    	//console.log ('SalesMonthFctEditExt:load eOpts = ' + eOpts);
	    	//console.log ('SalesMonthFctEditExt:load store = ' + store);
	    	//console.log ('SalesMonthFctEditExt:load store.grid = ' + store.grid);
	    	//console.log ('SalesMonthFctEditExt:load records = ' + records);
	    	if(records == null || records.length == 0)
	    		return;
	    	var startIdx = records[0].data.startHalf*6;// multyply idx by 0 or 1 to start either from Jan or July
	    	var countDown= (startIdx==0)?12:6; 
